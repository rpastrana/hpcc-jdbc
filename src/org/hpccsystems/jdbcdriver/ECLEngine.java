...
import org.hpccsystems.jdbcdriver.HPCCColumnMetaData.COLUMN_TYPE;
...
public class ECLEngine
{
    ...
    private void generateSelectECL() throws SQLException
    {
        ...
        for (int i = 0; i < expectedretcolumns.size(); i++)
        {
            ...
            if (col.getColumnType() == COLUMN_TYPE.CONSTANT)
            {
            ...
            else if (col.getColumnType() == COLUMN_TYPE.FNCTION)
            {
                ...
                else if (col.getColumnName().equalsIgnoreCase("MAX"))
                {
                    ...
                    if (funccols.size() > 0)
                    {
                        ...
                        if (!paramname.equals("*") && funccols.get(0).getColumnType() != COLUMN_TYPE.CONSTANT)
                        {
                            selectStructSB.append(", ");
    ...
    private boolean containsPayload(Properties indexfields, Iterator<HPCCColumnMetaData> selectcolsit)
    {
        while (selectcolsit.hasNext())
        {
            ...
            HPCCColumnMetaData.COLUMN_TYPE type = currentselectcol.getColumnType();
            if (type == COLUMN_TYPE.FIELD && !indexfields.containsKey(colname.toUpperCase()))
                return false;
            else if (type == COLUMN_TYPE.FNCTION  && !containsPayload(indexfields, currentselectcol.getFunccols().iterator()))
                return false;
        }
        return true;
    }
