...
/*
                ...
                //HPCCResultSet res = (HPCCResultSet) stmt.executeQuery("select min(persons.firstname), persons.firstname, persons.lastname from tutorial::rp::tutorialperson as persons where persons.firstname < 'a' limit 10");

              HPCCResultSet res = (HPCCResultSet) stmt.executeQuery(
                        "select  count( persons.city), city  from tutorial::rp::tutorialperson as persons where  persons.city  > 'DELRAY' group by city having count(city) < 32 order by city ASC limit 100");


                if (res.getRowCount() > 0)
                    printOutResultSet(res, Thread.currentThread().getId());
